...
class SensorData:
    ...
    def __init__(self, sensor_type, time, sensor_time, x, y, z):
        ...
        self.time = time # timestamps are in milliseconds
        ...
        self.sensor_time = np.arange(len(sensor_time)) # timestamps are in milliseconds
        
        self.x = sensor_time.astype(float)
        self.y = time.astype(float)
        self.z = x.astype(float)
...
class Trial:
    ...
    def __init__(self, gesture_name, trial_num, log_filename_with_path):
        ...
        parsed_accel_log_data = np.genfromtxt(log_filename_with_path, delimiter=',', 
                              encoding=None, skip_header=1, unpack=True)
        print(parsed_accel_log_data)
